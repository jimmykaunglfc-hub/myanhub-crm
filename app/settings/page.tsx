...
export default function EnhancedSettings() {
  ...
  // Controls whether we are viewing the list of connected accounts OR adding a new one
  ...
  useEffect(() => {
    ...
    const initializeSettings = async () => {
      ...
      if (session?.user) {
        ...
        if (profileError) console.error("Profile load restricted by security node:", profileError.message);

        if (profile) {
          if (profile.currency_code) setCurrencyCode(profile.currency_code.trim().toUpperCase());
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('profiles').update({ business_name: businessName, phone: phoneNumber }).eq('id', userId);
  ...
  const toggleAiPilot = async () => {
    ...
    const { error } = await supabase.from('profiles').update({ ai_auto_respond: newState }).eq('id', userId);
    if (error) {
  ...
  const handleSaveChannelConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We no longer update the existing token. We ALWAYS insert a new one to support multiple accounts.
      ...
      setChannelStatus(`Success! New account established for ${activeChannel}.`);
      setCredentialKey('');
      setIsEditing(false); // Return to list view
  ...
  const handleDisconnect = async (integrationId: string, channel: string) => {
    ...
    setChannelStatus(`Successfully removed account from ${channel}.`);
  };
  ...
  const guidelines: Record<string, { title: string; steps: string[]; placeholder: string }> = {
    telegram: { title: "Telegram Bot API", steps: ["Open Telegram and search for @BotFather.", "Send '/newbot' and assign a new public handle.", "Copy the generated HTTP API Token string.", "Paste the token values below to activate webhook."], placeholder: "Enter Bot API Token" },
    viber: { title: "Viber Business Chat", steps: ["Visit the official Viber Partner Console.", "Select 'Create Bot Account' and fill out details.", "Acquire your master App Token key.", "Bind your account parameters to the MyanHub node."], placeholder: "Enter Viber App Token Key" },
    tiktok: { title: "TikTok Shop API", steps: ["Sign in directly to your TikTok Developer portal.", "Authorize 'MyanHub Platform Connector'.", "Extract your App Key and Client Secret.", "Paste your keys below to begin inventory sync."], placeholder: "Enter TikTok Shop App Key" },
    whatsapp: { title: "WhatsApp Cloud API", steps: ["Go to Meta Developer Matrix portal.", "Set up WhatsApp Business API access triggers.", "Generate your permanent authorization token.", "Submit the key below to pipeline communication."], placeholder: "Enter WhatsApp Access Token" },
    line: { title: "Line Developers Connect", steps: ["Log into Line Developers Business console.", "Create a Messaging API Provider channel.", "Issue a permanent Long-Lived Access Token.", "Feed the token string into your MyanHub receiver."], placeholder: "Enter Line Access Token" }
  };

  // Get ALL integrations for the currently selected channel
  const activeChannelIntegrations = activeChannel ? integrations.filter(i => i.channel === activeChannel) : [];
      ...
      <main className="flex-1 md:ml-64 flex flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto mt-16 p-6 md:p-8 space-y-8 max-w-6xl mx-auto w-full pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className={`p-5 rounded-xl flex flex-col justify-between transition-colors border ${isDarkMode ? 'bg-slate-950/50 border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-100 hover:border-slate-200'}`}>
                <div>
                  ...
                  <span className={`text-xs leading-relaxed block mb-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Toggle Sandbox environments or switch to Live relays.</span>
                </div>
              ...
              <div className={`p-5 rounded-xl flex flex-col justify-between transition-colors border ${isDarkMode ? 'bg-slate-950/50 border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-100 hover:border-slate-200'}`}>
                <div>
                  ...
                  <span className={`text-xs leading-relaxed block mb-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Flip workspace styles to Light skin or Midnight mode.</span>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-slate-800">
              ...
              <div className={`p-5 rounded-xl flex flex-col justify-between transition-colors border ${isDarkMode ? 'bg-slate-950/50 border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-100 hover:border-slate-200'}`}>
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <span className={`text-xs leading-relaxed block ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Set the default currency for your inventory and orders.</span>
                  </div>
                ...
                <div className="relative mt-2">
                  <select value={currencyCode} onChange={(e) => handleCurrencyChange(e.target.value)} className={`w-full appearance-none pl-4 pr-10 py-2.5 rounded-lg text-sm font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all border cursor-pointer ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
                    {isSavingCurrency ? <CheckCircle2 size={16} className="text-emerald-500 animate-in zoom-in" /> : <span className="text-slate-400 text-xs">▼</span>}
                  </div>
                </div>
              </div>
              ...
              <div className={`p-5 rounded-xl flex flex-col justify-between transition-all border ${aiEnabled ? 'shadow-[0_0_15px_rgba(99,102,241,0.25)] border-indigo-500/50' : ''} ${isDarkMode ? 'bg-indigo-950/10' : 'bg-indigo-50/50'}`}>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className={`p-2 rounded-lg flex-shrink-0 bg-indigo-600 text-white ${aiEnabled ? 'animate-pulse' : ''}`}><Bot size={18} /></div>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-indigo-500/20">
                  <span className={`text-xs font-black uppercase tracking-wider ${aiEnabled ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`}>{aiEnabled ? 'ONLINE' : 'OFFLINE'}</span>
          ...
          <div className={`p-6 md:p-8 rounded-2xl border shadow-sm transition-colors duration-200 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            ...
            <p className={`text-xs mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Select a commercial provider below to integrate directly into your workspace node.</p>
            ...
            {activeChannel && (
              <div className={`mt-6 p-6 border rounded-xl animate-fade-in transition-colors ${isDarkMode ? 'bg-slate-950/80 border-indigo-500/30' : 'bg-indigo-50/50 border-indigo-100'}`}>
                ...
                {/* 1. LIST CONNECTED ACCOUNTS VIEW */}
                {activeChannelIntegrations.length > 0 && !isEditing ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between border-b pb-4 border-indigo-500/20">
                      <div className="flex items-center gap-3 text-emerald-600 dark:text-emerald-400">
                        ...
                        <div>
                          ...
                          <p className="text-xs opacity-80 mt-0.5">{activeChannelIntegrations.length} account{activeChannelIntegrations.length > 1 ? 's' : ''} currently connected.</p>
                        </div>
                      </div>
                      <button 
                        onClick={() => setIsEditing(true)} 
                        className="px-4 py-2 rounded-lg text-xs font-bold transition bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm flex items-center gap-1"
                      >
                        + Add Another
                      </button>
                    </div>
                    ...
                    <div className="space-y-3">
                      {activeChannelIntegrations.map((integration, index) => (
                        <div key={integration.id} className={`flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between p-4 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                          <div className={`flex-1 p-3 rounded-lg font-mono text-sm border flex items-center gap-3 w-full ${isDarkMode ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                            <KeyRound size={16} className="text-slate-400 flex-shrink-0" />
                            <span className="truncate">Account {index + 1}: {maskToken(integration.token)}</span>
                          </div>
                          
                          <div className="flex items-center gap-3 w-full sm:w-auto">
                            <button onClick={() => handleDisconnect(integration.id, activeChannel)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 border border-rose-500/20">
                    ...
                    {activeChannel === 'facebook' ? (
                      <div className={`border rounded-xl p-6 max-w-md ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            ...
                            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Connect an additional Facebook page.</p>
                          </div>
                        </div>
                        <button
                        ...
                        {activeChannelIntegrations.length > 0 && (
                      ...
                      <>
                        <div className="flex items-start gap-3 text-indigo-600">
                          ...
                          <div>
                            <h4 className={`text-sm font-bold ${isDarkMode ? 'text-indigo-300' : 'text-indigo-900'}`}>Add New {guidelines[activeChannel].title}</h4>
                            <p className={`text-xs mt-1 leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Generate a new token for your additional account and paste it below.</p>
                          </div>
                        </div>
